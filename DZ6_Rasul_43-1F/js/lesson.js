...
let cardIndex = 1; // Начинаем с первой карточки
const maxCardIndex = 200; // Максимальное количество карточек

// DRY: Функция для загрузки данных карточки
...
// DRY: Функция для загрузки всех постов и отображения в консоли
...
nextButton.onclick = () => {
    cardIndex = cardIndex === maxCardIndex ? 1 : cardIndex + 1; // Циклический переход
    loadCard(cardIndex);
};

// Обработчик нажатия кнопки Prev
prevButton.onclick = () => {
    cardIndex = cardIndex === 1 ? maxCardIndex : cardIndex - 1; // Циклический переход
    loadCard(cardIndex);
};

// Инициализация: загрузка первой карточки и лог постов в консоли
loadCard(cardIndex);
logPosts();
